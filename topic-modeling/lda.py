    ...
    def report(self, vocab, handle, limit=25):
        ...
        for kk in self._normalizer:
            ...
            for ww in self._topic_term[kk]:
                l.append(self.word_in_topic(kk,ww))
                w.append(ww)
            w = [x for _,x in sorted(zip(l,w), reverse=True)]
...
class Sampler:
    ...
    def change_topic(self, doc, index, new_topic):
        ...
        term = self._doc_tokens[doc][index] # index=n --> the nth word in doc
        alpha = self._alpha
        ...
        old_topic = self._doc_assign[doc][index]  # the topic of the nth word
    ...
    def run_sampler(self, iterations = 100):
        ...
        for ii in range(iterations):
            ...
            l.append(total)
            ############################################################
        print("Total usage of time: %0.5f seconds" % sum(l))
        ################################################################

    def report_topics(self, vocab, outputfilename, limit=49):
        """
        ...
        """
        topicsfile = open(outputfilename + "topics.txt", 'w')
    ...
    def sample_probs(self, doc_id, index):
        ...
        sample_probs = {} #dict
        term = self._doc_tokens[doc_id][index] #a word

        
        for kk in range(self._num_topics):
            
            #sample_probs[kk] = self._topics._topic_term[kk][term] + self._topics._default_beta
        ####################################################
            #sample_probs[kk] = self.lhood()
            #sample_probs[kk] = 1.0 / float(self._num_topics)
            sample_probs[kk] = ((self._topics._topic_term[kk][term] + self._topics._default_beta) / (sum(self._topics._topic_term[kk]) + self._topics._beta_sum)) * ((self._doc_counts[doc_id][kk] + self._alpha[kk]) / (sum(self._doc_counts[doc_id]) + sum(self._alpha)))

            #sample_probs[kk] *= ((self._doc_counts[doc_id][kk] + self._alpha[kk]) / (sum(self._doc_counts[doc_id]) + sum(self._alpha)))

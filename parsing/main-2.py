...
#from model_c1 import ParserModel_c1
...
def train(save_dir='saved_weights',
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i, (train_x, train_y) in enumerate(get_minibatches([dataset.train_inputs,
            ...
            #word_inputs_batch = torch.from_numpy(np.array(word_inputs_batch))
            #word_inputs_batch.to(device)
            #pos_inputs_batch = torch.from_numpy(np.array(pos_inputs_batch))
            #pos_inputs_batch.to(device)
            #dep_inputs_batch = torch.from_numpy(np.array(dep_inputs_batch))
            #dep_inputs_batch.to(device)
            word_inputs_batch=torch.tensor(word_inputs_batch, dtype=torch.long, device=device)
            pos_inputs_batch=torch.tensor(pos_inputs_batch, dtype=torch.long, device=device)
            dep_inputs_batch=torch.tensor(dep_inputs_batch, dtype=torch.long, device=device)
            ...
            # function in numpy.
            labels = np.array([np.argmax(idx) for idx in train_y])
            ...
            acc_store.append(int((outputs.argmax(1)==labels).sum())/len(labels))
        ...
        print('Saving the loss and accuracy data..')
        np.savetxt("loss1.csv", loss_store, delimiter=",", header = 'loss')
        np.savetxt("acc1.csv", acc_store, delimiter=",", header = 'acc')
